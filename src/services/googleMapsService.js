...
exports.buscarEstabelecimentosGoogle = async (location, radius, type) => {
  try {
    ...
    do {
      ...
      if (nextPageToken) {
        await new Promise((resolve) => setTimeout(resolve, 2000)); // tempo necessário para o token ativar
        url = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?pagetoken=${nextPageToken}&key=${GOOGLE_API_KEY}&language=pt-BR`;
      } else {
        console.log("Nenhuma próxima página disponível.");
      }

    } while (nextPageToken && resultados.length < 60);

    console.log(`Busca finalizada com ${resultados.length} resultados totais.`);
    return resultados;
